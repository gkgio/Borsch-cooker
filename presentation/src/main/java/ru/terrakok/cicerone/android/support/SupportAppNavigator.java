...
import com.gkgio.borsch_cooker.main.BottomBarTabsSwitcher;
...
public class SupportAppNavigator implements Navigator {
    ...
    private void fragmentForwardTo(SwitchTo command) {
        ...
        if (command.getId() == R.id.tab_own) {
            fragment.switchToOwnTab();
        } else if (command.getId() == R.id.tab_meals) {
            fragment.switchToMealsTab();
        } else if (command.getId() == R.id.tab_orders) {
            fragment.switchToOrdersTab();
